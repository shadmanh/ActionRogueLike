...
void UValInteractionComponent::FindBestInteractable()
{
	...
	if (FocusedActor)
	{
		...
		{
			DefaultWidgetInstance = CreateWidget<UValWorldUserWidget>(GetWorld(), DefaultWidgetClass);
		}
...
void UValInteractionComponent::ServerInteract_Implementation(AActor* InFocus)
{
	if (InFocus == nullptr)
	{
	...
	IValGameplayInterface::Execute_Interact(InFocus, MyPawn);
